  ...
  <div class="grid grid-cols-7 gap-2">
    <div v-for="day in days" :key="day" class="flex flex-col gap-1.5">
      <span class="text-xs font-semibold text-center"
        :class="schedule.find(s => s.date === day) ? 'text-zinc-900' : 'text-zinc-300'">
        {{ day }}
      </span>
      <template v-if="schedule.filter(e => e.date === day).length">
        <ScheduleBlock
          v-for="event in schedule.filter(e => e.date === day)"
          :key="event.timeStart"
          :event="event"
        />
      </template>
