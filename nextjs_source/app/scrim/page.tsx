...
const Entry = ({
  ...
  return (
    <div className={`${containerClasses} overflow-hidden`}>
      <div
        className={`${contentClasses} overflow-hidden relative flex flex-row gap-3 items-center h-16 p-4 z-20 select-none cursor-pointer`}
        onClick={toggleOpen}
      >
        ...
        <img
          src={map.thumbnail}
          alt="Map Type Icon"
          className="absolute object-cover w-full z-0 left-0 scale-105 blur-[1px]"
        />
        <img
          src={mapTypeIcons[map.type]}
          alt="Map Type Icon"
          className="z-20 h-full drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.5)]"
        />
        ...
        <div className="flex flex-row gap-2 z-20">
          {heroBans.map(hero => (
            <img
              ...
              src={hero.thumbnail}
              alt={hero.name}
              className="h-12 z-20 relative rounded-lg border-2 border-red-500 saturate-40"
            />
          ))}
        </div>
      ...
      >
        <div className="flex flex-row justify-between items-center">
          <div className="flex flex-col gap-0">
            <p className="text-3xl text-white font-black drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.5)]">
              {myTeamName}
            </p>
            <p className="text-2xl text-white font-bold drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.5)]">
              vs. {enemyTeamName}
            </p>
          </div>
          <div>
            <p className="text-6xl text-white font-black drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.5)]">
              {myTeamSR}
            </p>
          </div>
        </div>
        ...
        <p className="w-full text-center text-white font-bold drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.5)]">
          gress.tmp.ooo
        </p>
      </div>
