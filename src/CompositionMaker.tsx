...
import CircuitRoyal from "./assets/images/maps/circuit_royal.webp"
import { cn } from "./lib/utils"

const HeroSelectorThumbnail = ({ src, alt, className = "", onClick }: { src: string; alt: string; className?: string; onClick: () => void }) => {
  const classNames = cn(
    "rounded-lg relative bg-white overflow-hidden border border-gray-400/30 select-none",
...
const HeroSelector = ({ heroes, selectedHeroes, onClickFn, limitReached }: { heroes: IHeroData[], selectedHeroes: IHeroData[], onClickFn: (hero: IHeroData) => void, limitReached: boolean }) => {
  return (
    <div className="grid max-lg:gap-1 gap-2 max-lg:grid-cols-[repeat(5,1fr)] grid-cols-[repeat(8,1fr)] [&>*]:aspect-square">
      {
        heroes.map((hero) => {
...
const CompositionMaker = () => {
  ...
  return (
    <div className="flex flex-col gap-10">
      <h1 className="text-[50px] xl:text-[50px] lg:text-[50px] font-bold instrument-serif">Composition Picker</h1>
...
export default CompositionMaker;